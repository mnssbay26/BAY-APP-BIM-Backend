  ...
  try {
    ...
    if (!items.length) {
      return res.status(404).json({
        error: "Not Found",
        message: "No users found for the specified account/project/service",
      });
    }

    const contextText = JSON.stringify(items, null, 2);
    const systemPrompt = `
You are an expert BIM 360 Users assistant. You will receive a JSON array of user records, each containing these fields:
...
• Provide counts as numbers.
• Provide lists as arrays of user objects or IDs.
...
${question}
    ...
    console.error("❌ Error calling AI (Users):", err.message);
